...
## (merge all possible domain record lookups in one page. dont make seperate tools. list all detected records in the same page. allow user to pick which records to include, by default all of them.) ALSO merge all DNS tools in one page. theres currently a DNS Lookup tool, maybe rename it to DNS Tools, and all all others in it. you can make tabs in it or just stack all tools on top of each other, whichever works best.
...
--ok-- ## (in image tools) QR Scanner : QR Code Reader with Webcam or upload, show scan result in clickable a href link, scan in real time using video feed, if cdn necessary, download it and put it in js folder so that no internet is necessary to use the tool. for upload, show extracted content in immutable divbelow the upload button, and make it clickable if url or email or number etc.
--ok-- ## Lorem Ipsum: Generate Lorem Ipsum Text of given length.
--ok-- ## Credit Card Checker: Check if CC Number is valid -- other tools
--ok-- ## Online notepad tool that allows underline, bold, italic, strikethrough, tab (4 spaces), untab (shift+tab) etc., and lets user download as txt file OR whatever they rename it to (they can rename to .py or .html etc.). Allow opening text based files from local to edit and save them (as a different file), allow multiple file upload and open them as different tabs. infinite scroll, notepad background with lines. 20 mb size open limit. multiple tabs. users can rename a tab, and downloading downloads with that name. adda bulk download button that downloads all tabs as zip. [NOTE: implemented as plain text only — rich text formatting (bold/italic/etc.) is not possible in downloadable plain-text files]
--ok-- ## Cool stylized text generator using special alt characters, make the result copy pasteable, offer multiple variations. this will for example ask user for text input and give them a version of their text written with similar but weird characters, like the small versions of the same characters, or decorated, etc. al of which should be unicode.
--ok-- ## Common text art copy paste clipboard : Allows user to rapidly search for and copy commonly used text/punctuation art like smileys and more advanced stuff too. give names to things, allow search by name and characters, length filter etc.
